...
func (t Teacher) Join(db *gorm.DB) ([]Joined, error) {
	joined := []Joined{}
	err := db.Table("teachers").Select("teachers.name as teacher_name, schools.name as school_name, classes.name as class_name, lessons.name as lesson_name").Joins("join schools on teachers.school_id = schools.id").Joins("join classes on teachers.class_id = classes.id").Joins("join lessons on teachers.lesson_id = lessons.id").Scan(&joined).Error
	if err != nil {
		return []Joined{}, err
	}
	return joined, nil
